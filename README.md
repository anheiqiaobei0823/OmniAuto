...
> OmniAuto 是一个 AI API 智能路由网关。一个入口，接入所有模型。
...
1. **统一入口** —— 对外只暴露一个 `OmniAuto` 模型名和一个 API 地址，客户端不用管后面有多少供应商
...
客户端只看到 "OmniAuto" 一个模型，后面的一切对它是透明的。
```
...
OmniAuto 对外的模型名是 `OmniAuto`（也兼容 `auto` 或留空），接口兼容 OpenAI 格式：
...
如果你知道要用哪个内部模型，可以直接指定模型名而非 `OmniAuto`：
...
- **ChatBox / NextChat / LobeChat** —— 填 API 地址和 Key，选 `OmniAuto` 模型即可
- **Kelivo** —— 同上
- **Cursor / Continue** —— 填 API Base URL + Key，模型名填 `OmniAuto`
...
- 最新源码：直接 `git clone` 本仓库
- 历史版本：在 [Releases](https://github.com/anheiqiaobei0823/OmniAuto/releases) 页面选择对应版本下载 `OmniAuto-vX.Y.Z.tar.gz`
